...
| {{EVIDENCE_KIND}} | 证据类型；agent-runner 输出必须为 template，完成实质分析的人工或 specialist 报告使用 specialist | template, specialist |
...
agent-runner 生成的文件是 automation-generated evidence/template，不等同于已完成的 specialist review，并且必须声明 `evidence_kind: template`。报告必须由对应智能体或人工补充实质分析、影响、行动项和置信度并改为 `evidence_kind: specialist` 后，才能作为 advisory、blocking、merge 或 release 决策依据。`model_policy` 只说明调度时推荐的模型层级和推理强度；它不能替代 dated evidence、测试输出、来源链接或质量门禁。
...
前三项应由 `node scripts/planning-evidence.mjs` 在同一 evidence run 中生成。GitHub Issues 获取失败时命令必须 fail closed，并保留已有的有效 issue snapshot；测试可通过 `--issues-fixture` 注入确定性 issue 状态而不依赖 live secrets。
