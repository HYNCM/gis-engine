...
function applyProviderCommands(engine, output, spec) {
  ...
  switch (action) {
    ...
    case "setLayout":
      if (layerId && paint)
        commands = [{ type: "setLayout", layerId, layout: paint }];
      break;
    case "setView":
      if (view?.center && view?.zoom != null)
        commands = [{ type: "setView", view }];
      break;
    case "addLayer":
      if (layer?.id && layer?.type && layer?.source)
        commands = [{ type: "addLayer", layer }];
      break;
    case "removeLayer":
    ...
    case "reset":
      ...
      return {
        ...
        evidence: { committed: true, rolledBack: false, changedPathCount: 1 },
      };
    default:
  ...
  if (commands.length === 0) {
    return {
      status: "ready",
      nextSpec: spec,
      evidence: { committed: false, rolledBack: false, changedPathCount: 0 },
    };
  }
  ...
  try {
    ...
    return {
      ...
      evidence: {
        ...
        changedPathCount:
          result.results?.filter((r) => r.status === "applied").length || 0,
      },
    };
  } catch (err) {
...
function applyLegacyIntent(engine, intent, spec) {
  ...
  else if (msg.includes("larger") || msg.includes("bigger"))
    plan = {
      intent: "setPaint",
      layerId: "points-layer",
      paint: {
        "circle-radius": Math.min(
          30,
          (spec.layers[0]?.paint?.["circle-radius"] || 8) + 4,
        ),
      },
    };
  else if (msg.includes("smaller") || msg.includes("decrease"))
    plan = {
      intent: "setPaint",
      layerId: "points-layer",
      paint: {
  ...
  else if (msg.includes("hangzhou") || msg.includes("zoom"))
    plan = { intent: "setView", view: { center: [120.155, 30.274], zoom: 13 } };
  else if (msg.includes("reset")) plan = { intent: "reset" };
  // Basemap
  else if (msg.includes("osm") || msg.includes("openstreetmap"))
    plan = { intent: "setBasemap", layerId: "osm" };
  else if (msg.includes("dark") || msg.includes("carto dark"))
    plan = { intent: "setBasemap", layerId: "carto-dark" };
  else if (msg.includes("light") || msg.includes("carto light"))
    plan = { intent: "setBasemap", layerId: "carto-light" };
  else if (msg.includes("satellite") || msg.includes("imagery"))
    plan = { intent: "setBasemap", layerId: "satellite" };
  ...
  if (plan.intent === "setBasemap") {
    const bmId =
      plan.layerId && BASEMAPS[plan.layerId] ? plan.layerId : DEFAULT_BASEMAP;
    activeBasemap = bmId;
    const bm = BASEMAPS[bmId];
    commands = [
      { type: "removeSource", sourceId: BASEMAP_SOURCE_ID },
...
function statePayload(engine, status, spec) {
  ...
  const filtered = allDiags.filter((d) => {
    if (
      d.code === "SECURITY.URL_BLOCKED" &&
      d.path?.includes(BASEMAP_SOURCE_ID)
    )
      return false;
    if (d.code === "SPEC.UNKNOWN_FIELD" && d.path?.includes(BASEMAP_SOURCE_ID))
      return false;
    return true;
  });
  return {
    ...
    try {
      ...
      if (req.method === "GET" && url.pathname === "/api/basemaps") {
        return sendJson(res, {
          ...
          options: Object.entries(BASEMAPS).map(([id, bm]) => ({
            id,
            label: bm.label,
          })),
        });
      ...
      if (req.method === "POST" && url.pathname === "/api/chat") {
        ...
        if (providerId === "deepseek") {
          ...
          const cmdResult = applyProviderCommands(
            engine,
