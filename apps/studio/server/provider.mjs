...
export async function callOpenAiCompatibleProvider(input) {
  ...
  if (!apiKey)
    return providerError(profile, "Provider credential is not configured.");
  ...
  try {
    ...
    if (!parsed.ok)
      return providerError(profile, "Response must be a JSON object.");
...
function hashPrompt(msg) {
  return `sha256:${createHash("sha256").update(msg).digest("hex").slice(0, 16)}`;
}
function sanitizeConfidence(c) {
  ...
  return score != null
    ? { score, level: score > 0.7 ? "high" : score > 0.4 ? "medium" : "low" }
    : undefined;
}
function providerError(profile, msg) {
  return { ok: false, error: `${profile.id}: ${msg}` };
}
