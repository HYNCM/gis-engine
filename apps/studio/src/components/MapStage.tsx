...
export default function MapStage({ serverState, status, onSave, savedMsg, basemaps, currentBasemap, onChangeBasemap }: Props) {
  const containerRef = useRef<HTMLDivElement>(null);
  const mapRef = useRef<maplibregl.Map | null>(null);

  useEffect(() => {
    if (!containerRef.current || mapRef.current) return;
    const map = new maplibregl.Map({ container: containerRef.current, style: { version: 8, sources: {}, layers: [] }, center: [120.15, 30.28], zoom: 10 });
  ...
  useEffect(() => {
    ...
    const style = serverState.style as maplibregl.StyleSpecification;
  ...
  }, [serverState?.style, serverState?.summary.revision]);

  const badgeColor = status === "ready" || status === "applied" ? "bg-green-900/50 text-green-400" : status === "thinking" ? "bg-yellow-900/50 text-yellow-400" : "bg-red-900/50 text-red-400";
    ...
    <div className="relative w-full h-full">
      ...
      <div className="absolute top-0 left-0 right-0 flex items-center justify-between px-4 py-2 bg-gray-900/80 backdrop-blur border-b border-gray-800">
        ...
        <div className="flex items-center gap-2">
          <button onClick={onSave} disabled={!serverState} className="text-xs px-3 py-1 rounded bg-gray-800 hover:bg-gray-700 text-gray-300 disabled:opacity-40 transition" title="Save">💾 Save</button>
          <span className={`text-xs px-2 py-0.5 rounded-full ${badgeColor}`}>{status}</span>
        </div>
      </div>
      <div className="absolute bottom-4 left-4 bg-gray-900/80 backdrop-blur rounded px-3 py-1.5 text-xs text-gray-400 font-mono">MapSpec v0.1 · MapLibre GL</div>
    </div>
  );
}
