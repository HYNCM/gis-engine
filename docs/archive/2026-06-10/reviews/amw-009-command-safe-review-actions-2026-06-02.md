...
| Audit alignment | accepted | `docs/archive/2026-06-10/feature-specs/ai-map-workbench-durable-audit.md` reserves `reviewOutcome` after AMW-009 while keeping audit payload-free. | Review outcomes can attach to compact audit records without expanding raw-payload retention. | Durable storage/export still requires a separate schema and authorization task. | high |
| Outcome semantics | accepted as design | `docs/archive/2026-06-10/feature-specs/ai-map-workbench-review-actions.md` defines `accepted`, `blocked`, and `follow-up-required` outcome rules. | Review-console users get decision semantics before product promotion. | Reuse generated-app delivery states rather than adding MCP aliases. | high |
