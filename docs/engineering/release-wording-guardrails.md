...
| Cloud-native sources | Say GeoJSON/raster/vector/PMTiles are bounded by the current source matrix. PMTiles has URL-compatible MapLibre vector display Go and IO-free load-plan/source-readiness preflight Go; runtime archive load and runtime feature query are No-go with stable blocker codes. PMTiles runtime `queryReady` stays false; point/bbox fixture evidence is separate `fixtureEvidenceReady`/`fixtureEvidenceStatus` and remains payload-free. GeoParquet, FlatGeobuf, and GeoTIFF are public `MapSpec` source contracts with runtime blocked, and GeoZarr remains blocked. | Do not claim runtime-blocked or blocked formats have active runtime loaders, parsers, workers, runtime feature queries, sampling, display promotion, or export mutation support. The exported `PMTilesRuntimeLoader` is a fail-closed compatibility shell, not runtime support. |
...
- "PMTiles is URL-compatible display/export evidence, and point/bbox query
  evidence can be fixture-only with caller-supplied decoded features; archive
  parsing and runtime feature query stay follow-up work."
