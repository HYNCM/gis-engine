...
| Experimental boundary | `fill-extrusion-lite`, `scene3d` | `fill-extrusion-lite` is schema-gated but not rendered by the current MapLibre MVP; `scene3d` is reserved and returns unsupported diagnostics. |
