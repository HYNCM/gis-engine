...
| AI-native next loop | planned | `docs/planning/sprint-2026-W22-ai-native-next-loop.md`; next tasks start from generated-app delivery UX, acceptance states, source promotion candidates, spatial-analysis promotion criteria, and scene browsing copy |
...
2. Generation quality hardening -> typed prompt planner boundary -> planner provenance evidence -> spatial query evidence -> export manifest -> cloud-native readiness -> SceneView3D blocker transparency -> serialized closure. NLQ-001 through NLQ-007 are done; future work now starts from the AIN next-loop sprint rather than mutating this closed batch.
