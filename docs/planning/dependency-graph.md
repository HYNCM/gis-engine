...
| SceneView3D stable renderer contract | partial done / stable blocked | `SRC-001`, `SRC-003`, and `SRC-004` have adapter/QA contract evidence; `SRC-002` and `SRC-005` remain todo; `SRC-006` remains blocked until quality-guardian/coordinator accept a future stable runtime decision |
