...
| Data and sources | source readiness summary, resource-policy result, source diagnostics | GeoJSON/raster/vector/PMTiles states match existing policy and examples | GeoParquet/FlatGeobuf/GeoTIFF public schema-policy contracts, GeoZarr, and new PMTiles archive behavior remain readiness-only or blocked until promotion gates land |
...
| GeoParquet | Readiness-only source card with public contract evidence. | Do not claim runtime loading or query; the card must point to future promotion gates for read-only query evidence and runtime support. |
| FlatGeobuf | Readiness-only source card with public contract evidence. | Do not claim runtime loading or query; the card must point to future promotion gates for read-only query evidence and runtime support. |
| GeoTIFF | Readiness-only source card with public contract evidence. | Do not claim runtime loading, display promotion, sampling, or query; the card must point to future raster decode/display evidence, byte/range policy, band/CRS/no-data diagnostics, worker policy, and snapshot gates. |
