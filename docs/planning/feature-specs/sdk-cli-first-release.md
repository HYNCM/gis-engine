...
- `bundle-size.yml` adds hard budgets: engine < 50KB gz, cli < 20KB gz.
...
- `--prompt` flag + `GIS_ENGINE_PROMPT` env var for prompt text
...
- Config parsing: 18 tests (defaults, flags, short flags, equals form, env vars, combined)
- Provider diagnostics: 5 tests (mock, known, unknown, case-insensitive)
...
- CLI package size: 10.8KB (well under 20KB gz budget)
