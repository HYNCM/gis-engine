...
当前 v0.1 已具备 `MapSpec`、commands、diagnostics、snapshot 和 MapLibre adapter 闭环；2026-05-17 v0.2 checkpoint 已完成 generic vector tile、MCP output schema、strict capability report、expression 扩展、style/layer order 稳定化、visual MVT 场景和 2.5D/3D 边界。2026-05-18 已补齐 package dry-run、resource/perf deterministic evidence 和 command audit trace 产品化。下一阶段不应继续扩大 runtime 骨架，而应聚焦正式 runner 发布证据、2.5D beta 和 3D 边界深化。
...
| v0.2.x | 协作安全 + 2.5D beta | command audit trace、resource/perf evidence、fill-extrusion-lite beta adapter | 多 agent 编辑不破坏回放一致性，2.5D 不污染核心 schema |
...
| 2 | fill-extrusion-lite beta adapter | 5.85 | core capabilities、3D roadmap | 仅在 capability gate 后实验性映射 MapLibre `fill-extrusion` | medium |
| 3 | SceneView3D 边界深化 | 5.30 | Cesium/Three.js signal、framework review | 继续细化 scene3d/camera/source/layer/snapshot 边界 spec | medium |
| 4 | 大数据 perf/nightly evidence | 4.80 | resource/perf gap plan | 将 1k/10k/100k 场景放入 nightly/release runner | medium |
...
2. 继续保持 2.5D/3D 为 extension/capability-gated，不提前进入核心 renderer 承诺。
...
| `docs/planning/feature-specs/fill-extrusion-lite-beta.md` | boundary done / beta pending | experimental gate、snapshot contract、fallback diagnostics |
