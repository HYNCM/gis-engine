...
| TASK-2026W23-AWP-002 | Implement provider resource enforcement | P0 | M | `@ai-agent`, `@engine-agent`, `@qa-agent` | queued | AWP-001, AMW-007 | Server-side base URL policy, timeout/abort, response byte cap, stable diagnostics, and browser-safe provider metadata are enforced without moving the workbench out of `examples/`. | provider/workbench tests; leak regression tests; `pnpm test:examples`; `pnpm check`; `git diff --check` |
...
`TASK-2026W23-AWP-001` is complete as planning evidence. The next queued task is
`TASK-2026W23-AWP-002`, and it must stay limited to provider resource
enforcement inside the current local/example boundary.
