...
| TASK-2026W23-SQH-002 | Add explicit query capability gate | P0 | M | `@engine-agent`, `@ai-agent` | todo | SQH-001 | Generation/query evidence names adapter query capability or an explicit waiver before ready state. | `pnpm build:schema` if schemas change; `pnpm test:commands`; `pnpm test:ai`; `pnpm check` |
...
`TASK-2026W23-SQH-002` should enter execution state next. It is the first code
slice and must avoid widening into PMTiles/vector parsing, new MCP tools, or
