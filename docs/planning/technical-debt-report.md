...
| --- | ---: | ---: | --- | --- | --- |
| 1 | AWP provider resource enforcement | 0.18 | `AWP-001` opens the product implementation loop and assigns `AWP-002` before durable audit or review-action runtime | implement server-side base URL policy, timeout/abort, response byte cap, stable diagnostics, and leak regression tests | high |
...
1. 执行 `TASK-2026W23-AWP-002`：在当前 local/example 边界内实现 provider resource enforcement，不移动 product app、不加 hosted deployment、不新增 MCP tool name。
...
如果只做一件事，下一步优先执行 `TASK-2026W23-AWP-002`。Generated-app delivery
...
实现批次。SceneView3D stable runtime 仍保持 blocker state，直到新的 explicit
approval arrives。
