...
当前债务已经从“合同漂移”转为“发布证据和长期能力边界”。两个 P1 合同缺口已关闭，v0.2 checkpoint 已完成；package 发布检查、deterministic resource/perf 证据和 command audit trace 产品化已补齐。剩余风险主要集中在正式 release runner、2.5D beta、3D 后续边界和大数据 nightly 证据。
...
| 2 | fill-extrusion-lite 仍只有 boundary | 5.20 | feature spec、schema gate | 未来 beta adapter 明确 capability 后再实现渲染与 visual evidence | medium |
| 3 | SceneView3D 仍为 reserved boundary | 4.90 | scene3d-boundary spec | v1 前继续细化 camera/source/layer/snapshot 规则 | medium |
| 4 | 大数据 perf/nightly evidence 未固化 | 4.80 | resource/perf gap plan | 将 1k/10k/100k 场景放入 nightly/release runner，而非 PR blocker | medium |
