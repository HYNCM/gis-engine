...
**建议**: 立即执行 `TASK-2026W22-MLD-002`（已在 task-burndown 中定义为 P0）。
...
| ID | 升级项 | 优先级 | 动作 |
| --- | --- | --- | --- |
| UP-001 | MapLibre source drift audit | **P0** | 执行 `TASK-2026W22-MLD-002`：审计 5.24.0 → v6 的 transformer、source URL、resource policy、snapshot 影响 |
...
| ID | 升级项 | 优先级 | 动作 |
| --- | --- | --- | --- |
| UP-003 | SourceLoader contract | **P1** | 在 engine 中定义 `SourceLoader` 接口和 `SourceLifecycle` 枚举，MapLibre adapter 实现之。不改变现有行为，仅暴露 contract。 |
...
| 新 renderer adapter 需要重做 source loading | 低（短期）→ 高（长期） | 高（重复工作） | UP-003 SourceLoader contract |
...
**如果只做一件事**: 执行 `MLD-002`（MapLibre source drift audit）。

**升级优先级**:
1. MLD-002: MapLibre source drift audit（P0）
2. UP-002: 架构文档对齐（P0）
3. UP-003: SourceLoader contract（P1）
