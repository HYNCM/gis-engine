...
- `generationEvidence.delivery.sourceReadiness`: the visible readiness rail in
  the evidence sidebar, derived from the engine source-readiness report and
  showing query readiness, resource-policy status, runtime load-plan summaries,
  and archive contract snapshots when present.
