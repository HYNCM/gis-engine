  ...
  switch (sectionId) {
    ...
    case "files-and-export":
      return {
        ...
        confirmations: (delivery.confirmations ?? []).filter(
          (c) => c.reason === "file-write" || c.reason === "network-fetch",
        ),
      };
...
function computeSourcePromotionCandidates(
  delivery,
  sourceReadiness = normalizeSourceReadinessEntries(delivery.sourceReadiness ?? []),
) {
  ...
  if (Array.isArray(explicitCandidates) && explicitCandidates.length > 0) {
    return explicitCandidates.map((candidate) => ({
      ...
      resourcePolicy:
        ...
        sourceReadiness.find((source) => source.sourceId === candidate.sourceIds?.[0])?.resourcePolicy ??
        "not-checked",
      ...buildContractFields(
        candidate.format,
        candidate.archiveContract ??
          sourceReadiness.find((source) => source.sourceId === candidate.sourceIds?.[0])?.archiveContract,
        candidate.sourceContract ??
          sourceReadiness.find((source) => source.sourceId === candidate.sourceIds?.[0])?.sourceContract,
      ),
    }));
  }
