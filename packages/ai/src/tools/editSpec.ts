...
function parseInstruction(instruction: string, spec: MapSpec): ParsedInstruction | null {
  ...
  if (addLayerMatch) {
    ...
    const effectiveType: string = layerType === "heatmap" ? "circle" : layerType;

    const layer = {
      id: layerId,
      type: effectiveType as "circle",
    ...
    return {
      ...
      summary: `Added ${effectiveType} layer "${layerId}"${sourceId ? ` for source "${sourceId}"` : ""}.`,
    };
  }
      ...
      commands: [
        {
          ...
          type: "setFilter",
          layerId,
          filter: filter as import("@gis-engine/engine").Expression,
        },
      ],
...
function parseFilterExpression(raw: string): import("@gis-engine/engine").Expression {
  const eqMatch = /^(\S+)\s+equals?\s+(.+)$/i.exec(raw);
  if (eqMatch) {
    const field = eqMatch[1] ?? "";
    const value = parseValue((eqMatch[2] ?? "").trim());
    return ["==", ["get", field], value] as JsonValue[];
  }
