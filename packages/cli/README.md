...
| `src/App.tsx` | Map container, status banner, optional `delivery-summary.json` and `artifact-manifest.json` review status/details with preflight/source/query/follow-up signals, artifact file summary, and local `map.json` reload/upload flow that mounts the generated spec and UI components. |
