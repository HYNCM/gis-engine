...
`;
  ...
  for (const [complexity, data] of Object.entries(d1.byComplexity || {})) {
    const status =
      data.avgDeviation <= 0.5
        ? "✓"
        : data.avgDeviation <= 1.0
          ? "🟡"
          : "🔴";
    content += `
...
priority_formula:
  ...
  w2_ai_operability: ${d6.weights?.w2 || 0.30}     # was: 0.30
  w3_user_value: ${d6.weights?.w3 || 0.20}         # was: 0.20
  w4_tech_debt: ${d6.weights?.w4 || 0.10}          # was: 0.10
...
**Next Review**: Next sprint coordinator summary
`;
