...
describe("MapLibre exact-version compatibility audit", () => {
  it("keeps 5.24.0 as the release baseline and treats stable 6.1.0 as an adoption candidate", () => {
    const report = runMapLibreV6Audit();

    expect(report.checkedVersions).toEqual(["5.24.0", "6.1.0"]);
  ...
  it("records peer, ESM/type, event, and strict visual evidence separately", () => {
    ...
    expect(byId.get("stable-peer-range")?.severity).toBe("pass");
    expect(byId.get("public-adapter-types")?.severity).toBe("pass");
    expect(byId.get("esm-generated-example")?.severity).toBe("pass");
    expect(byId.get("adapter-lifecycle-events")?.severity).toBe("pass");
    expect(byId.get("missing-image-query-semantics")?.severity).toBe("pass");
    expect(byId.get("overscaled-vector-query")?.severity).toBe("pass");
