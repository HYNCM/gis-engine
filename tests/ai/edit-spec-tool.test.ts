...
describe("edit_spec MCP tool", () => {
  ...
  it("adds heatmap layer as heatmap instead of degrading to circle", () => {
    const result = editSpecTool({
      spec: baseSpec(),
      instruction: "add a heatmap layer density for source cities",
    });

    expect(result.ok).toBe(true);
    if (result.ok) {
      const newLayer = result.result.spec.layers.find((layer) => layer.id === "density");
      expect(newLayer).toMatchObject({
        id: "density",
        type: "heatmap",
        source: "cities",
      });
      expect(newLayer?.paint?.["heatmap-radius"]).toBe(24);
    }
  });

  it("returns an error when a source-backed layer edit would make the spec invalid", () => {
    const result = editSpecTool({
      spec: baseSpec(),
      instruction: "add fill layer parks",
    });
  ...
  it("generates deterministic command ids for the same edit", () => {
    const input = {
      spec: baseSpec(),
      instruction: "set zoom to 8",
    };

    const first = editSpecTool(input);
    const second = editSpecTool(input);

    expect(first.ok).toBe(true);
    expect(second.ok).toBe(true);
    if (first.ok && second.ok) {
      expect(first.result.commands).toEqual(second.result.commands);
    }
  });
