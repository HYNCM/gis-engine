...
describe("nightly performance evidence", () => {
  it.each(
    scales,
  )("keeps create/query/snapshot/destroy within nightly budgets for $featureCount inline features", async ({
    featureCount,
    createBudgetMs,
    queryBudgetMs,
    snapshotBudgetMs,
...
function buildFeatures(featureCount: number): Array<{
  type: "Feature";
    ...
    geometry: {
      ...
      coordinates:
        index === 0
          ...
          : ([10 + index * 0.00001, 10 + index * 0.00001] as [number, number]),
    },
  }));
